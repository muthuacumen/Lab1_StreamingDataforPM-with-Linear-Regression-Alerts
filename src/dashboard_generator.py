"""
Dashboard Generator for Predictive Maintenance.

Generates a self-contained interactive HTML dashboard from alert_log.csv
using Plotly.js (CDN) with embedded data. No additional Python dependencies required.

Designed for Plant Maintenance Engineers -- uses plain operational language,
actionable insights, and avoids statistical jargon.
"""
...
def generate_html_dashboard(alert_log_path, output_html_path, robot_names=None):
    """
    Read alert_log.csv and generate a self-contained interactive HTML dashboard.

    Parameters
    ----------
    alert_log_path : str
        Path to the alert_log.csv file.
    output_html_path : str
        Path where the HTML dashboard will be written.
    robot_names : list of str, optional
        Full list of monitored robot names. If provided, robots with 0 events
        still appear in summary cards, health table, and charts.

    Returns
    -------
    str or None
        Absolute path to the generated HTML file, or None if no data.
    """
    # Load alert log
    try:
        df = pd.read_csv(alert_log_path)
    except FileNotFoundError:
        print(f"Alert log not found: {alert_log_path}")
        return None

    if df.empty:
        print("Alert log is empty -- no dashboard generated.")
        return None

    # Round float columns to reduce HTML size
    ...
    # Build robot names list: use provided list, else derive from data
    if robot_names is None:
        robot_names = sorted(df['robot'].unique().tolist())
    robots_json = json.dumps(sorted(robot_names))

    # Generation timestamp
    gen_time = datetime.now().strftime("%b %d, %Y at %I:%M %p")

    # Build HTML from template
    html_content = _build_html(data_json, gen_time, robots_json)
...
def _build_html(data_json, gen_time, robots_json):
    """Build the complete HTML string with embedded data."""

    # Using string.Template with $variable syntax to avoid JS {} conflicts
...
<header>
  ...
  <p class="timestamp">Report generated: $gen_time</p>
</header>
...
renderEventTable();
    ...
    return template.substitute(data_json=data_json, gen_time=gen_time, robots_json=robots_json)


if __name__ == '__main__':
    # Standalone test
    log_path = os.path.join(os.path.dirname(__file__), '..', 'logs', 'alert_log.csv')
    out_path = os.path.join(os.path.dirname(__file__), '..', 'alerts', 'alert_dashboard.html')
    all_robots = ['Robot A', 'Robot B', 'Robot C', 'Robot D']
    result = generate_html_dashboard(log_path, out_path, robot_names=all_robots)
    if result:
